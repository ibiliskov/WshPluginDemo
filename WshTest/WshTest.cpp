...
struct ScriptInterface : public IDispatch {
    ...
    long  WINAPI GetIDsOfNames( REFIID riid, WCHAR** name, UINT cnt ,LCID lcid, DISPID *id)
    {
        for(DWORD j=0; j<cnt;j++)
        {
            ...
            {
                // Check app functions
				if(strcmp(buf+k,"GetVal")==0) { id[j]=(DISPID)GetVal; break ; } else
				if(strcmp(buf+k,"SetVal")==0) { id[j]=(DISPID)SetVal; break ; } else
				if(strcmp(buf+k,"Invalidate")==0) { id[j]=(DISPID)Invalidate; break ; } else
				if(strcmp(buf+k,"Debug")==0) { id[j]=(DISPID)Debug; break ; } else
