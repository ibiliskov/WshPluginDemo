...
struct ScriptInterface : public IDispatch {
	...
	long  WINAPI GetIDsOfNames(REFIID riid, WCHAR** name, UINT cnt, LCID lcid, DISPID* id)
	{
		for (DWORD j = 0; j < cnt; j++)
		{
			...
			{
				// Check app functions
				if (strcmp(buf + k, "GetVal") == 0) { id[j] = (DISPID)GetVal; break; }
				else
					if (strcmp(buf + k, "SetVal") == 0) { id[j] = (DISPID)SetVal; break; }
					else
						if (strcmp(buf + k, "Invalidate") == 0) { id[j] = (DISPID)Invalidate; break; }
						else
							if (strcmp(buf + k, "Debug") == 0) { id[j] = (DISPID)Debug; break; }
							else

								// win32 api in known dlls
								for (int i = 0; (dll[i] || dll[i - 1]); i++)
								{
									if ((id[j] = (DISPID)GetProcAddress(dll[i], buf + k))) break;
								}
				if (id[j]) break;
			}
			cdcl = k;
	...
	long  WINAPI Invoke(DISPID id, REFIID riid, LCID lcid, WORD flags, DISPPARAMS* arg, VARIANT* ret, EXCEPINFO* excp, UINT* err) {
		...
		if (id) {
			...
			while ((cdcl ? (--i > -1) : (++i < arg->cArgs)))
			{
				...
				{
					int d = (int)arg->rgvarg[i].dblVal; param = (DWORD)d;
				}
				_asm push param;
			}
			...
			if (ret) ret->vt = VT_UI4; __try { if (!c[1] && (*c < '0' || *c>'9')) ret->vt = VT_BSTR; ret->bstrVal = SysAllocString((WCHAR*)c); }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	wchar_t* name[] = { L"ntdll",L"msvcrt",L"kernel32",L"user32",L"advapi32",L"shell32",L"wsock32",L"wininet",L"<",L">",0 };
	for (int i = 0; name[i]; i++) dll[i] = LoadLibrary(name[i]);

	// Set language
	GUID guid; CLSIDFromProgID(L"JavaScript", &guid);

	hr = CoCreateInstance(guid, 0, CLSCTX_ALL, IID_IActiveScript, (void**)&script);  if (!script) return 1;
